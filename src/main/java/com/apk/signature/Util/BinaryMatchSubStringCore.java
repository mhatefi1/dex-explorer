...
public class BinaryMatchSubStringCore {
    ...
    private boolean getStringAddressFromHexString(byte[] stream, int ids_count, String text, Item tClass, int min, int max) {
        ...
        if (min < ids_count) {
            ...
            for (int i = min; i <= max; i++) {
                long offset = ids_offset + (long) tClass.data_size * i;
                boolean ss = searchDataByteContainSubString(stream, offset, splitByte);
                if (ss) {
                    if (!dexNamePrinted) {
                        print(dexName + ":");
                        dexNamePrinted = true;
    ...
    private boolean searchDataByteContainSubString(byte[] stream, long start, byte[] splitByte) {
        ...
        int i = 0;
        byte[] match_state = new byte[splitByte.length];
        while (true) {
            byte a_string_bit_in_MUTF8_format_b = util.getBytesOfFile(stream, offset, 1)[0];
            if (a_string_bit_in_MUTF8_format_b == 0) {
                break;
            } else {
                if (a_string_bit_in_MUTF8_format_b == splitByte[i]) {
                    match_state[i] = 1;
                    i++;
                    if (i == splitByte.length) break;
                } else {
                    match_state = new byte[splitByte.length];
                    i = 0;
                }
            }
            offset++;

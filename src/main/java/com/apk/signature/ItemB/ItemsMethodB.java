...
import com.apk.signature.Items.ItemsProto;
import com.apk.signature.Items.ItemsString;
import com.apk.signature.Items.ItemsType;
...
public class ItemsMethodB extends ItemB {
    ...
    public boolean searchDataByte(HashMap<String, byte[]> header, ByteArrayInputStream stream, long start, String[] splitText) {
        ...
        byte[] class_ = appUtil.getByteByIndex(header, stream, class_idx, new ItemsTypeB());
        ...
        if (contains == 0) {
            ...
            byte[] proto = appUtil.getByteByIndex(header, stream, proto_idx, new ItemsProtoB());
            ...
            if (contains1 == 0) {
                ...
                byte[] method = appUtil.getByteByIndex(header, stream, method_idx, new ItemsStringB());
    ...
    public String getDataAsHex(HashMap<String, byte[]> header, ByteArrayInputStream stream, long start) {
        ...
        byte[] class_idx_b = Arrays.copyOfRange(method_data_b, 0, 1);
        long class_idx = util.getDecimalValue(class_idx_b);
        String class_ = appUtil.getHexByIndex(header, stream, class_idx, new ItemsTypeB());

        byte[] proto_idx_b = Arrays.copyOfRange(method_data_b, 2, 3);
        long proto_idx = util.getDecimalValue(proto_idx_b);
        String proto = appUtil.getHexByIndex(header, stream, proto_idx, new ItemsProtoB());

        byte[] method_idx_b = Arrays.copyOfRange(method_data_b, 4, 7);
        long method_idx = util.getDecimalValue(method_idx_b);
        String method = appUtil.getHexByIndex(header, stream, method_idx, new ItemsStringB());
